#!/bin/bash
# Migration: Create Support Group Queue
# This migration creates the Support Group queue for incident management

# Source database configuration
MIGRATION_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
source "$MIGRATION_DIR/db_config.sh"

echo "  Creating Support Group queue..."
...
DO \$\$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM queue WHERE name = 'Support Group') THEN
        INSERT INTO queue (
            ...
            'Support Group', $USERS_GROUP_ID, 0, 240, 480, 
            1440, 1, 0, 1, 
            1, 1, 'Main support queue for all incidents', 1, CURRENT_TIMESTAMP,
            1, CURRENT_TIMESTAMP, 1
        );
        RAISE NOTICE 'Created Support Group queue';
    ELSE
        RAISE NOTICE 'Support Group queue already exists';
    END IF;
END
\$\$;
EOF
...
if [ $? -ne 0 ]; then
    echo "Error: Failed to create Support Group queue"
    exit 1
fi

echo "  Done! Created Support Group queue."
