...
# Copyright (C) 2025 MSST - Modified to add license checking
# --
...
sub Run {
    ...
    # MSST License Check - Block API access if license is invalid
    ...
    my $BlockAPI = $ConfigObject->Get('LicenseCheck::BlockAPI') // 1;  # Default to 1 if not configured
    
    # Debug logging
    $LogObject->Log(
        Priority => 'notice',
        Message  => "MSST API License Check: Enabled=$LicenseCheckEnabled, BlockAPI=$BlockAPI",
    );
    
    if ( $LicenseCheckEnabled && $BlockAPI ) {
        ...
        if ($Success) {
            ...
            # Block API if license is not valid
            if ($LicenseStatus ne 'Valid') {
                $LogObject->Log(
                    Priority => 'notice',
                    Message  => "Blocking API access due to invalid license. Status: $LicenseStatus",
                );
                ...
                print "Content-Length: " . length('{"Error":"Access denied. Invalid or expired license.","LicenseStatus":"' . $LicenseStatus . '"}') . "\r\n";
                print "\r\n";
                print '{"Error":"Access denied. Invalid or expired license.","LicenseStatus":"' . $LicenseStatus . '"}';
                
                return;
            }
        }
    }
    # End of MSST License Check
