...
package Kernel::Modules::AgentEscalatedClosedTicket;
...
sub Run {
    ...
    $Output .= $LayoutObject->Output(
        TemplateFile => 'AgentEscalatedClosedTicket',
        Data         => \%GetParam,
    );
...
sub _GetHourlyData {
    ...
    # Query escalated closed ticket COUNT grouped by hour and priority
    # Escalated = ticket has MSITicketNumber populated
    # Closed = ts.type_id = 3 (closed state type)
    my $SQL = q{
        SELECT
            EXTRACT(HOUR FROM t.change_time) as hour,
            tp.name as priority,
        ...
        WHERE t.type_id = 2
          ...
          AND dfv_msi.value_text IS NOT NULL AND dfv_msi.value_text != ''
          AND t.change_time >= ?
          AND t.change_time <= ?
        GROUP BY EXTRACT(HOUR FROM t.change_time), tp.name, tp.id
        ORDER BY hour, tp.id
    };
...
sub _GetDailyData {
    ...
    # Query escalated closed ticket COUNT grouped by date and priority
    my $SQL = q{
        SELECT
            DATE(t.change_time) as day,
            tp.name as priority,
            COUNT(*) as ticket_count
        FROM ticket t
        JOIN ticket_priority tp ON t.ticket_priority_id = tp.id
        ...
        WHERE t.type_id = 2
          ...
          AND t.change_time >= ?
          AND t.change_time <= ?
        GROUP BY DATE(t.change_time), tp.name, tp.id
        ORDER BY day, tp.id
    };
...
sub _GetWeeklyData {
    ...
    # Query escalated closed ticket COUNT grouped by week and priority
    my $SQL = q{
        SELECT
            EXTRACT(WEEK FROM t.change_time) as week,
            tp.name as priority,
            COUNT(*) as ticket_count
        FROM ticket t
        JOIN ticket_priority tp ON t.ticket_priority_id = tp.id
        ...
        WHERE t.type_id = 2
          ...
          AND t.change_time >= ?
          AND t.change_time <= ?
        GROUP BY EXTRACT(WEEK FROM t.change_time), tp.name, tp.id
        ORDER BY week, tp.id
    };
...
sub _GetMonthlyData {
    ...
    # Query escalated closed ticket COUNT grouped by month and priority
    my $SQL = q{
        SELECT
            EXTRACT(MONTH FROM t.change_time) as month,
            tp.name as priority,
            COUNT(*) as ticket_count
        FROM ticket t
        JOIN ticket_priority tp ON t.ticket_priority_id = tp.id
        ...
        WHERE t.type_id = 2
          ...
          AND t.change_time >= ?
          AND t.change_time <= ?
        GROUP BY EXTRACT(MONTH FROM t.change_time), tp.name, tp.id
...
sub _GetQuarterlyData {
    ...
    # Query escalated closed ticket COUNT grouped by quarter and priority
    my $SQL = q{
        SELECT
            EXTRACT(QUARTER FROM t.change_time) as quarter,
            tp.name as priority,
            COUNT(*) as ticket_count
        FROM ticket t
        JOIN ticket_priority tp ON t.ticket_priority_id = tp.id
        ...
        WHERE t.type_id = 2
          ...
          AND t.change_time >= ?
          AND t.change_time <= ?
        GROUP BY EXTRACT(QUARTER FROM t.change_time), tp.name, tp.id
...
sub _GetYearlyData {
    ...
    # Query escalated closed ticket COUNT grouped by year and priority
    my $SQL = q{
        SELECT
            EXTRACT(YEAR FROM t.change_time) as year,
            tp.name as priority,
            COUNT(*) as ticket_count
        FROM ticket t
        JOIN ticket_priority tp ON t.ticket_priority_id = tp.id
        ...
        WHERE t.type_id = 2
          ...
          AND t.change_time >= ?
          AND t.change_time <= ?
        GROUP BY EXTRACT(YEAR FROM t.change_time), tp.name, tp.id
        ORDER BY year, tp.id
    };
