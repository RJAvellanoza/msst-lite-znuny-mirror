...
package Kernel::Modules::AgentMTTR;
...
sub _GetHourlyData {
    ...
    # Query MTTR (time to resolution) grouped by hour and priority
    # Resolution time = first time ticket moved to closed state (type_id = 3) WITHIN the date range
    # This ensures tickets reopened and re-resolved are counted in the correct period
    # Fallback to change_time for tickets without StateUpdate history (e.g., created directly in closed state)
    # Filter by RESOLUTION DATE (when ticket was resolved), not creation date
    my $SQL = q{
        WITH first_resolution_in_range AS (
            ...
            WHERE th.history_type_id = 27
              AND ts_hist.type_id = 3 AND ts_hist.name != 'cancelled'
              AND th.create_time >= ?
              AND th.create_time <= ?
            GROUP BY th.ticket_id
        )
        SELECT
            EXTRACT(HOUR FROM COALESCE(fr.resolved_time, t.change_time)) as hour,
            tp.name as priority,
            SUM(EXTRACT(EPOCH FROM (COALESCE(fr.resolved_time, t.change_time) - t.create_time)) / 3600) as mttr_hours,
        ...
        GROUP BY EXTRACT(HOUR FROM COALESCE(fr.resolved_time, t.change_time)), tp.name, tp.id
        ORDER BY hour, tp.id
    };

    my @Bind = (\$StartDate, \$EndDate, \$StartDate, \$EndDate);

    # Initialize data structure for 24 hours
    my %HourData;
    my %GrandTotal = ( P1 => 0, P2 => 0, P3 => 0, P4 => 0 );
    my %TicketCount = ( P1 => 0, P2 => 0, P3 => 0, P4 => 0 );

    # Parse just the date portion for building per-hour date ranges
    my ($DateOnly) = split / /, $StartDate;
    ...
    for my $hour (0..23) {
        $HourData{$hour} = {
            ...
            P3 => 0,
            P4 => 0,
            StartDate => sprintf("%s %02d:00:00", $DateOnly, $hour),
            EndDate => sprintf("%s %02d:59:59", $DateOnly, $hour),
        };
    }
    ...
    while ( my @Row = $DBObject->FetchrowArray() ) {
        my $Hour = int($Row[0]);
        my $Priority = $Row[1];
        my $MTTRHours = sprintf("%.2f", $Row[2] || 0);
        my $Count = $Row[3] || 0;
        ...
        if ( $PriorityKey && exists $HourData{$Hour} ) {
            $HourData{$Hour}{$PriorityKey} = $MTTRHours;
            $GrandTotal{$PriorityKey} += $MTTRHours;
            $TicketCount{$PriorityKey} += $Count;
        }
    }
    ...
    return (
        ...
        GrandTotal => \%GrandTotal,
        TicketCount => \%TicketCount,
        GrandTotalStartDate => $StartDate,
        GrandTotalEndDate => $EndDate,
    );
}
...
sub _GetDailyData {
    ...
    # Query MTTR grouped by date and priority
    # Resolution within date range - ensures reopened tickets are counted correctly
    ...
    my $SQL = q{
        ...
        SELECT
            DATE(COALESCE(fr.resolved_time, t.change_time)) as day,
            tp.name as priority,
            SUM(EXTRACT(EPOCH FROM (COALESCE(fr.resolved_time, t.change_time) - t.create_time)) / 3600) as mttr_hours,
            COUNT(*) as ticket_count
        FROM ticket t
        ...
        GROUP BY DATE(COALESCE(fr.resolved_time, t.change_time)), tp.name, tp.id
        ORDER BY day, tp.id
    };
    ...
    for my $i (0..6) {
        ...
        $DayData{$date_str} = {
            ...
            P3 => 0,
            P4 => 0,
            StartDate => "$date_str 00:00:00",
            EndDate => "$date_str 23:59:59",
        };
    }
    ...
    while ( my @Row = $DBObject->FetchrowArray() ) {
        ...
        if ( $PriorityKey && exists $DayData{$Day} ) {
            $DayData{$Day}{$PriorityKey} = $MTTRHours;
            $GrandTotal{$PriorityKey} += $MTTRHours;
            $TicketCount{$PriorityKey} += $Count;
        }
    }
...
sub _GetWeeklyData {
    ...
    # Query MTTR grouped by week and priority
    # Resolution within date range - ensures reopened tickets are counted correctly
    # Fallback to change_time for tickets without StateUpdate history
    # Filter by RESOLUTION DATE (when ticket was resolved), not creation date
    my $SQL = q{
        ...
        SELECT
            EXTRACT(WEEK FROM COALESCE(fr.resolved_time, t.change_time)) as week,
            tp.name as priority,
            SUM(EXTRACT(EPOCH FROM (COALESCE(fr.resolved_time, t.change_time) - t.create_time)) / 3600) as mttr_hours,
            COUNT(*) as ticket_count
        FROM ticket t
        ...
        GROUP BY EXTRACT(WEEK FROM COALESCE(fr.resolved_time, t.change_time)), tp.name, tp.id
        ORDER BY week, tp.id
    };
    ...
    while ( my @Row = $DBObject->FetchrowArray() ) {
        ...
        if ( !exists $WeekData{$Week} ) {
            ...
            my ($YearFromDate) = split /-/, $StartDate;
            use POSIX qw(mktime);
            my $jan4 = mktime(0, 0, 0, 4, 0, $YearFromDate - 1900);
            my @jan4_date = localtime($jan4);
            my $jan4_dow = ($jan4_date[6] + 6) % 7;
            my $first_monday = $jan4 - ($jan4_dow * 86400);
            my $week_start_time = $first_monday + (($Week - 1) * 7 * 86400);
            my $week_end_time = $week_start_time + (6 * 86400);
            my @ws = localtime($week_start_time);
            my @we = localtime($week_end_time);
            my $week_start_str = sprintf("%04d-%02d-%02d 00:00:00", $ws[5]+1900, $ws[4]+1, $ws[3]);
            my $week_end_str = sprintf("%04d-%02d-%02d 23:59:59", $we[5]+1900, $we[4]+1, $we[3]);
        ...
        if ( $PriorityKey ) {
            $WeekData{$Week}{$PriorityKey} = $MTTRHours;
            $GrandTotal{$PriorityKey} += $MTTRHours;
            $TicketCount{$PriorityKey} += $Count;
        }
    }
...
sub _GetMonthlyData {
    ...
    # Query MTTR grouped by month and priority
    # Resolution within date range - ensures reopened tickets are counted correctly
    # Fallback to change_time for tickets without StateUpdate history
    # Filter by RESOLUTION DATE (when ticket was resolved), not creation date
    my $SQL = q{
        ...
        SELECT
            EXTRACT(MONTH FROM COALESCE(fr.resolved_time, t.change_time)) as month,
            tp.name as priority,
            SUM(EXTRACT(EPOCH FROM (COALESCE(fr.resolved_time, t.change_time) - t.create_time)) / 3600) as mttr_hours,
            COUNT(*) as ticket_count
        FROM ticket t
        ...
        GROUP BY EXTRACT(MONTH FROM COALESCE(fr.resolved_time, t.change_time)), tp.name, tp.id
    ...
    for my $month ($StartMonth..$EndMonth) {
        ...
        my $month_start_str = sprintf("%04d-%02d-01 00:00:00", $Year, $month);
        my $next_month_time_calc = mktime(0, 0, 0, 1, $month, $Year - 1900);
        my $last_day_time_calc = $next_month_time_calc - 1;
        my @ld = localtime($last_day_time_calc);
        my $month_end_str = sprintf("%04d-%02d-%02d 23:59:59", $ld[5]+1900, $ld[4]+1, $ld[3]);
    ...
    while ( my @Row = $DBObject->FetchrowArray() ) {
        ...
        if ( $PriorityKey && exists $MonthData{$Month} ) {
            $MonthData{$Month}{$PriorityKey} = $MTTRHours;
            $GrandTotal{$PriorityKey} += $MTTRHours;
            $TicketCount{$PriorityKey} += $Count;
        }
    }
...
sub _GetQuarterlyData {
    ...
    # Query MTTR grouped by quarter and priority
    # Resolution within date range - ensures reopened tickets are counted correctly
    # Fallback to change_time for tickets without StateUpdate history
    # Filter by RESOLUTION DATE (when ticket was resolved), not creation date
    my $SQL = q{
        ...
        SELECT
            EXTRACT(QUARTER FROM COALESCE(fr.resolved_time, t.change_time)) as quarter,
            tp.name as priority,
            SUM(EXTRACT(EPOCH FROM (COALESCE(fr.resolved_time, t.change_time) - t.create_time)) / 3600) as mttr_hours,
            COUNT(*) as ticket_count
        FROM ticket t
        ...
        GROUP BY EXTRACT(QUARTER FROM COALESCE(fr.resolved_time, t.change_time)), tp.name, tp.id
    ...
    # Quarter to month mapping: Q1=Jan-Mar, Q2=Apr-Jun, Q3=Jul-Sep, Q4=Oct-Dec
    my %QuarterMonths = (
        1 => [1, 3],   # Jan 1 - Mar 31
        2 => [4, 6],   # Apr 1 - Jun 30
        3 => [7, 9],   # Jul 1 - Sep 30
        4 => [10, 12], # Oct 1 - Dec 31
    );

    for my $quarter (1..4) {
        my ($start_month, $end_month) = @{$QuarterMonths{$quarter}};
        my $q_start = sprintf("%04d-%02d-01 00:00:00", $Year, $start_month);
        # Last day of end_month
        use POSIX qw(mktime);
        my $next_month_time_q = mktime(0, 0, 0, 1, $end_month, $Year - 1900);
        my $last_day_time_q = $next_month_time_q - 1;
        my @ld_q = localtime($last_day_time_q);
        my $q_end = sprintf("%04d-%02d-%02d 23:59:59", $ld_q[5]+1900, $ld_q[4]+1, $ld_q[3]);
    ...
    while ( my @Row = $DBObject->FetchrowArray() ) {
        ...
        if ( $PriorityKey && exists $QuarterData{$Quarter} ) {
            $QuarterData{$Quarter}{$PriorityKey} = $MTTRHours;
            $GrandTotal{$PriorityKey} += $MTTRHours;
            $TicketCount{$PriorityKey} += $Count;
        }
    }
...
sub _GetYearlyData {
    ...
    # Query MTTR grouped by year and priority
    # Resolution within date range - ensures reopened tickets are counted correctly
    # Fallback to change_time for tickets without StateUpdate history
    # Filter by RESOLUTION DATE (when ticket was resolved), not creation date
    my $SQL = q{
        ...
        SELECT
            EXTRACT(YEAR FROM COALESCE(fr.resolved_time, t.change_time)) as year,
            tp.name as priority,
            SUM(EXTRACT(EPOCH FROM (COALESCE(fr.resolved_time, t.change_time) - t.create_time)) / 3600) as mttr_hours,
            COUNT(*) as ticket_count
        FROM ticket t
        ...
        GROUP BY EXTRACT(YEAR FROM COALESCE(fr.resolved_time, t.change_time)), tp.name, tp.id
        ORDER BY year, tp.id
    };
    ...
    while ( my @Row = $DBObject->FetchrowArray() ) {
        ...
        if ( $PriorityKey && exists $YearData{$Year} ) {
            $YearData{$Year}{$PriorityKey} = $MTTRHours;
            $GrandTotal{$PriorityKey} += $MTTRHours;
            $TicketCount{$PriorityKey} += $Count;
        }
    }
