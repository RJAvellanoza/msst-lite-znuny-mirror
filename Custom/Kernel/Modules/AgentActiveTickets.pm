...
package Kernel::Modules::AgentActiveTickets;
...
sub Run {
    ...
    my $Output = $LayoutObject->Header(
        Title => 'Active Tickets Counts',
    );
    $Output .= $LayoutObject->NavigationBar();
    $Output .= $LayoutObject->Output(
        TemplateFile => 'AgentActiveTickets',
        Data         => \%GetParam,
    );
...
sub _GetHourlyData {
    ...
    # Only tickets that are currently NOT closed (ticket_state.type_id != 3)
    my $SQL = q{
        SELECT
...
sub _GetDailyData {
    ...
    # Query ACTIVE tickets grouped by date and priority
    my $SQL = q{
        SELECT
...
sub _GetWeeklyData {
    ...
    # Query ACTIVE tickets grouped by week and priority
    my $SQL = q{
        SELECT
    ...
    while ( my @Row = $DBObject->FetchrowArray() ) {
        ...
        if ( !exists $WeekData{$Week} ) {
            ...
            my $jan4_dow = ($jan4_date[6] + 6) % 7;
            my $first_monday = $jan4 - ($jan4_dow * 86400);
            my $week_start_ts = $first_monday + (($Week - 1) * 7 * 86400);
            my $week_end_ts = $week_start_ts + (6 * 86400);
            my @ws = localtime($week_start_ts);
            my @we = localtime($week_end_ts);
...
sub _GetMonthlyData {
    ...
    # Query ACTIVE tickets grouped by month and priority
    my $SQL = q{
        SELECT
    ...
    for my $month ($StartMonth..$EndMonth) {
        ...
        $MonthData{$month} = {
            ...
            EndDate => sprintf("%04d-%02d-%02d 23:59:59", $Year, $month, $last_day),
            P1 => 0,
            P2 => 0,
            P3 => 0,
            P4 => 0,
...
sub _GetQuarterlyData {
    ...
    # Query ACTIVE tickets grouped by quarter and priority
    my $SQL = q{
        SELECT
    ...
    for my $quarter (1..4) {
        my $start_month = $QuarterMonths{$quarter}{start};
        my $end_month = $QuarterMonths{$quarter}{end};
        $QuarterData{$quarter} = {
            Label => "Q$quarter",
            StartDate => sprintf("%04d-%02d-01 00:00:00", $Year, $start_month),
            EndDate => sprintf("%04d-%02d-%s 23:59:59", $Year, $end_month, $end_month == 3 || $end_month == 12 ? '31' : '30'),
            P1 => 0,
            P2 => 0,
            P3 => 0,
            P4 => 0,
...
sub _GetYearlyData {
    ...
    # Query ACTIVE tickets grouped by year and priority
    my $SQL = q{
        SELECT
