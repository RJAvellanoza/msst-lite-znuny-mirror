...
package Kernel::Modules::AdminUserNOC;
...
sub Run {
        ...
        )
    {
        ...
        # NOCAdmin restriction check
        if ($UserID) {
            my %TargetUserGroups = $GroupObject->PermissionUserGet(
                UserID => $UserID,
                Type   => 'rw',
            );
            my $AdminGroupID = $GroupObject->GroupLookup(Group => 'admin') || 0;
            my $NOCAdminGroupID = $GroupObject->GroupLookup(Group => 'NOCAdmin') || 0;
            my $NOCUserGroupID = $GroupObject->GroupLookup(Group => 'NOCUser') || 0;
            
            # Get target user data to check for special users
            my %TargetUserData = $UserObject->GetUserData(
                UserID => $UserID,
                NoOutOfOffice => 1,
            );
            
            # Special restriction for msifield users - cannot switch to msicmso accounts
            if ($Self->{UserLogin} eq 'msifield' && $TargetUserData{UserLogin} eq 'msicmso') {
                return $LayoutObject->ErrorScreen(
                    Message => Translatable('Permission Denied'),
                    Comment => Translatable('msifield users are prohibited from switching to msicmso user accounts.'),
                );
            }
            
            # Block if target is admin or not a NOC user
            if ($AdminGroupID && exists $TargetUserGroups{$AdminGroupID}) {
                return $LayoutObject->ErrorScreen(
                    Message => Translatable('Permission Denied'),
                    Comment => Translatable('You cannot switch to admin users. NOCAdmin can only manage NOC users.'),
                );
            }
            
            my $IsNOCUser = 0;
            if (($NOCAdminGroupID && exists $TargetUserGroups{$NOCAdminGroupID}) ||
                ($NOCUserGroupID && exists $TargetUserGroups{$NOCUserGroupID})) {
                $IsNOCUser = 1;
            }
            ...
            if (!$IsNOCUser) {
                return $LayoutObject->ErrorScreen(
                    Message => Translatable('Permission Denied'),
                    Comment => Translatable('You can only manage users in NOCAdmin or NOCUser groups.'),
                );
            }
        }
    ...
    # ------------------------------------------------------------ #
        ...
        if ($UserID) {
            ...
            # Special restriction for msifield users - cannot modify msicmso accounts
            if ($Self->{UserLogin} eq 'msifield' && $TargetUserData{UserLogin} eq 'msicmso') {
                return $LayoutObject->ErrorScreen(
                    Message => Translatable('Permission Denied'),
                    Comment => Translatable('msifield users are prohibited from making any changes to msicmso user accounts.'),
                );
            }
            ...
            if ($AdminGroupID && exists $TargetUserGroups{$AdminGroupID}) {
                return $LayoutObject->ErrorScreen(
                    Message => Translatable('Permission Denied'),
                    Comment => Translatable('You cannot edit admin users. NOCAdmin can only manage NOC users.'),
                );
            }
    ...
    # ------------------------------------------------------------ #
        ...
        if ($GetParam{UserID}) {
            ...
            if ($AdminGroupID && exists $TargetUserGroups{$AdminGroupID}) {
                return $LayoutObject->ErrorScreen(
                    Message => Translatable('Permission Denied'),
                    Comment => Translatable('You cannot modify admin users. NOCAdmin can only manage NOC users.'),
                );
            }
        ...
        # Special restriction for msifield users - cannot change login IDs
        if ($Self->{UserLogin} eq 'msifield') {
            # Get current user login to compare
            my $CurrentUserLogin = $UserObject->UserLookup(
                UserID => $GetParam{UserID},
            );
            if ($CurrentUserLogin && $CurrentUserLogin ne $GetParam{UserLogin}) {
                return $LayoutObject->ErrorScreen(
                    Message => Translatable('Permission Denied'),
                    Comment => Translatable('msifield users are prohibited from changing anyone\'s login ID (username).'),
                );
            }
        }
    ...
    # ------------------------------------------------------------ #
    # add - BLOCKED for NOCAdmin users
    # ------------------------------------------------------------ #
    elsif ( $Self->{Subaction} eq 'Add' ) {
        # NOCAdmin users cannot add new users
        return $LayoutObject->ErrorScreen(
            Message => Translatable('Permission Denied'),
            Comment => Translatable('NOCAdmin users cannot create new users. Please contact an administrator.'),
        );
        my %GetParam = ();
    ...
    # ------------------------------------------------------------ #
    # add action - BLOCKED for NOCAdmin users
    # ------------------------------------------------------------ #
...
sub _Overview {
    ...
    # NOCAdmin users should not add new agents - removed ActionAdd block
    ...
    my $GroupObject = $Kernel::OM->Get('Kernel::System::Group');
    my $AdminGroupID = $GroupObject->GroupLookup(Group => 'admin') || 0;
    my $MSIAdminGroupID = $GroupObject->GroupLookup(Group => 'MSIAdmin') || 0;

    # Filter the list to only include NOC users (exclude admin and MSIAdmin)
    my %FilteredList;
    for my $ListKey ( keys %List ) {
        ...
        # Skip if user is in admin or MSIAdmin groups
        next if ($AdminGroupID && exists $UserGroups{$AdminGroupID});
        next if ($MSIAdminGroupID && exists $UserGroups{$MSIAdminGroupID});

        # Add to filtered list
        $FilteredList{$ListKey} = $List{$ListKey};
    }

    # Update header with correct count of NOC users only
    my $FilteredCount = keys %FilteredList;
    $LayoutObject->Block(
        Name => 'OverviewHeader',
        Data => {
            ListAll => $FilteredCount,
            Limit   => $Limit,
        },
    );
    ...
    if (%FilteredList) {
        for my $ListKey ( sort { $FilteredList{$a} cmp $FilteredList{$b} } keys %FilteredList ) {
...
sub _EffectivePermissions {
    ...
    for my $GroupID ( sort { uc( $Groups{$a} ) cmp uc( $Groups{$b} ) } keys %Groups ) {
        ...
        # Skip MSIAdmin group - NOC users should not see MSIAdmin group assignments
        next if $Groups{$GroupID} eq 'MSIAdmin';
