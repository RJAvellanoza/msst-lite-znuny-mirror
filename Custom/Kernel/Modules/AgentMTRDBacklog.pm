...
package Kernel::Modules::AgentMTRDBacklog;
...
sub Run {
    ...
    my $Output = $LayoutObject->Header(
        Title => 'MTRD Breakdown',
    );
    $Output .= $LayoutObject->NavigationBar();
    $Output .= $LayoutObject->Output(
        TemplateFile => 'AgentMTRDBacklog',
        Data         => \%GetParam,
    );
...
sub _GetHourlyData {
    ...
    # Query MTRD (time to first assignment) grouped by hour and priority
    # SUM of MTRD hours and COUNT of tickets
    # Handles tickets created with owner (MTRD=0) and tickets assigned later
    # Filter by ASSIGNMENT DATE (when ticket was first assigned), not creation date
    my $SQL = q{
        ...
        SELECT
            EXTRACT(HOUR FROM foa.first_assigned_time) as hour,
            tp.name as priority,
            SUM(EXTRACT(EPOCH FROM (foa.first_assigned_time - t.create_time)) / 3600) as mtrd_hours,
            COUNT(*) as ticket_count
        FROM ticket t
        JOIN first_owner_assignment foa ON t.id = foa.ticket_id
        JOIN ticket_priority tp ON t.ticket_priority_id = tp.id
        WHERE t.type_id = 2
          AND foa.first_assigned_time >= ?
          AND foa.first_assigned_time <= ?
          AND foa.first_assigned_time IS NOT NULL
        GROUP BY EXTRACT(HOUR FROM foa.first_assigned_time), tp.name, tp.id
        ORDER BY hour, tp.id
    };
    ...
    while ( my @Row = $DBObject->FetchrowArray() ) {
        my $Hour = int($Row[0]);
        my $Priority = $Row[1];
        my $MTRDHours = sprintf("%.2f", $Row[2] || 0);
        my $Count = $Row[3] || 0;
        ...
        if ( $PriorityKey && exists $HourData{$Hour} ) {
            $HourData{$Hour}{$PriorityKey} = $MTRDHours;
            $GrandTotal{$PriorityKey} += $MTRDHours;
...
sub _GetDailyData {
    ...
    # Query MTRD grouped by date and priority - SUM of MTRD hours and COUNT
    # Handles tickets created with owner (MTRD=0) and tickets assigned later
    # Filter by ASSIGNMENT DATE (when ticket was first assigned), not creation date
    my $SQL = q{
        WITH first_owner_assignment AS (
        ...
        SELECT
            DATE(foa.first_assigned_time) as day,
            tp.name as priority,
            SUM(EXTRACT(EPOCH FROM (foa.first_assigned_time - t.create_time)) / 3600) as mtrd_hours,
            COUNT(*) as ticket_count
        FROM ticket t
        ...
        WHERE t.type_id = 2
          ...
          AND foa.first_assigned_time <= ?
          AND foa.first_assigned_time IS NOT NULL
        GROUP BY DATE(foa.first_assigned_time), tp.name, tp.id
        ORDER BY day, tp.id
    };
    ...
    while ( my @Row = $DBObject->FetchrowArray() ) {
        ...
        if ( $PriorityKey && exists $DayData{$Day} ) {
            $DayData{$Day}{$PriorityKey} = $MTRDHours;
            $GrandTotal{$PriorityKey} += $MTRDHours;
            $TicketCount{$PriorityKey} += $Count;
        }
    }
...
sub _GetWeeklyData {
    ...
    # Query MTRD grouped by week and priority - SUM of MTRD hours and COUNT
    # Handles tickets created with owner (MTRD=0) and tickets assigned later
    # Filter by ASSIGNMENT DATE (when ticket was first assigned), not creation date
    my $SQL = q{
        WITH first_owner_assignment AS (
        ...
        SELECT
            EXTRACT(WEEK FROM foa.first_assigned_time) as week,
            tp.name as priority,
            SUM(EXTRACT(EPOCH FROM (foa.first_assigned_time - t.create_time)) / 3600) as mtrd_hours,
            COUNT(*) as ticket_count
        FROM ticket t
        ...
        WHERE t.type_id = 2
          ...
          AND foa.first_assigned_time <= ?
          AND foa.first_assigned_time IS NOT NULL
        GROUP BY EXTRACT(WEEK FROM foa.first_assigned_time), tp.name, tp.id
        ORDER BY week, tp.id
    };
    ...
    while ( my @Row = $DBObject->FetchrowArray() ) {
        ...
        if ( $PriorityKey ) {
            $WeekData{$Week}{$PriorityKey} = $MTRDHours;
            $GrandTotal{$PriorityKey} += $MTRDHours;
            $TicketCount{$PriorityKey} += $Count;
        }
    }
...
sub _GetMonthlyData {
    ...
    # Query MTRD grouped by month and priority - SUM of MTRD hours and COUNT
    # Handles tickets created with owner (MTRD=0) and tickets assigned later
    # Filter by ASSIGNMENT DATE (when ticket was first assigned), not creation date
    my $SQL = q{
        WITH first_owner_assignment AS (
        ...
        SELECT
            EXTRACT(MONTH FROM foa.first_assigned_time) as month,
            tp.name as priority,
            SUM(EXTRACT(EPOCH FROM (foa.first_assigned_time - t.create_time)) / 3600) as mtrd_hours,
            COUNT(*) as ticket_count
        FROM ticket t
        ...
        WHERE t.type_id = 2
          ...
          AND foa.first_assigned_time <= ?
          AND foa.first_assigned_time IS NOT NULL
        GROUP BY EXTRACT(MONTH FROM foa.first_assigned_time), tp.name, tp.id
    ...
    while ( my @Row = $DBObject->FetchrowArray() ) {
        ...
        if ( $PriorityKey && exists $MonthData{$Month} ) {
            $MonthData{$Month}{$PriorityKey} = $MTRDHours;
            $GrandTotal{$PriorityKey} += $MTRDHours;
            $TicketCount{$PriorityKey} += $Count;
        }
    }
...
sub _GetQuarterlyData {
    ...
    # Query MTRD grouped by quarter and priority - SUM of MTRD hours and COUNT
    # Handles tickets created with owner (MTRD=0) and tickets assigned later
    # Filter by ASSIGNMENT DATE (when ticket was first assigned), not creation date
    my $SQL = q{
        WITH first_owner_assignment AS (
        ...
        SELECT
            EXTRACT(QUARTER FROM foa.first_assigned_time) as quarter,
            tp.name as priority,
            SUM(EXTRACT(EPOCH FROM (foa.first_assigned_time - t.create_time)) / 3600) as mtrd_hours,
            COUNT(*) as ticket_count
        FROM ticket t
        ...
        WHERE t.type_id = 2
          ...
          AND foa.first_assigned_time <= ?
          AND foa.first_assigned_time IS NOT NULL
        GROUP BY EXTRACT(QUARTER FROM foa.first_assigned_time), tp.name, tp.id
    ...
    while ( my @Row = $DBObject->FetchrowArray() ) {
        ...
        if ( $PriorityKey && exists $QuarterData{$Quarter} ) {
            $QuarterData{$Quarter}{$PriorityKey} = $MTRDHours;
            $GrandTotal{$PriorityKey} += $MTRDHours;
            $TicketCount{$PriorityKey} += $Count;
        }
    }
...
sub _GetYearlyData {
    ...
    # Query MTRD grouped by year and priority - SUM of MTRD hours and COUNT
    # Handles tickets created with owner (MTRD=0) and tickets assigned later
    # Filter by ASSIGNMENT DATE (when ticket was first assigned), not creation date
    my $SQL = q{
        WITH first_owner_assignment AS (
        ...
        SELECT
            EXTRACT(YEAR FROM foa.first_assigned_time) as year,
            tp.name as priority,
            SUM(EXTRACT(EPOCH FROM (foa.first_assigned_time - t.create_time)) / 3600) as mtrd_hours,
            COUNT(*) as ticket_count
        FROM ticket t
        ...
        WHERE t.type_id = 2
          ...
          AND foa.first_assigned_time <= ?
          AND foa.first_assigned_time IS NOT NULL
        GROUP BY EXTRACT(YEAR FROM foa.first_assigned_time), tp.name, tp.id
        ORDER BY year, tp.id
    };
    ...
    while ( my @Row = $DBObject->FetchrowArray() ) {
        ...
        if ( $PriorityKey && exists $YearData{$Year} ) {
            $YearData{$Year}{$PriorityKey} = $MTRDHours;
            $GrandTotal{$PriorityKey} += $MTRDHours;
            $TicketCount{$PriorityKey} += $Count;
        }
    }
