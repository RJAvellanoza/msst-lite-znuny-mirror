...
sub Run {
    ...
    # Get the 'new' state ID for filtering
    my $StateObject = $Kernel::OM->Get('Kernel::System::State');
    my %NewState = $StateObject->StateGet(
        Name => 'new',
    );
    ...
    my %Filters = (
        ...
        New => {
            Name   => Translatable('New Tickets'),
            Prio   => 1001,
            Search => {
                StateIDs => [ $NewState{ID} ],  # Only 'new' state tickets
                QueueIDs => \@ViewableQueueIDs,
                %Sort,
                Permission   => $Permission,
                UserID       => $Self->{UserID},
        ...
        MyTickets => {
            Name   => Translatable('My Tickets'),
            Prio   => 1002,
            Search => {
                OwnerIDs => [ $Self->{UserID} ],  # Only tickets owned by current user
                StateIDs => \@ViewableStateIDs,
                QueueIDs => \@ViewableQueueIDs,
                %Sort,
                Permission   => $Permission,
    ...
    my $Filter = $ParamObject->GetParam( Param => 'Filter' ) || 'New';  # Default to New instead of Unlocked
...
sub _MaskQueueView {
    ...
    for my $QueueRef (@ListedQueues) {
        ...
        my $Filter = $Kernel::OM->Get('Kernel::System::Web::Request')->GetParam( Param => 'Filter' ) || 'New';
