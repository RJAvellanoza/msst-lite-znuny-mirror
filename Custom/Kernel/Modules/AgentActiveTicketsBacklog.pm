...
package Kernel::Modules::AgentActiveTicketsBacklog;
...
sub Run {
    ...
    my $Output = $LayoutObject->Header(
        Title => 'Active Tickets Backlog',
    );
    $Output .= $LayoutObject->NavigationBar();
    $Output .= $LayoutObject->Output(
        TemplateFile => 'AgentActiveTicketsBacklog',
        Data         => \%GetParam,
    );
...
sub _GetHourlyData {
    ...
    # Query ACTIVE tickets grouped by hour and priority
    # SUM of backlog days and COUNT of tickets
    my $SQL = q{
        SELECT
            EXTRACT(HOUR FROM t.create_time) as hour,
            tp.name as priority,
            SUM(CURRENT_DATE - DATE(t.create_time)) as backlog_days,
    ...
    while ( my @Row = $DBObject->FetchrowArray() ) {
        ...
        if ( $PriorityKey && exists $HourData{$Hour} ) {
            $HourData{$Hour}{$PriorityKey} = $BacklogDays;
            $GrandTotal{$PriorityKey} += $BacklogDays;
...
sub _GetDailyData {
    ...
    # Query ACTIVE tickets grouped by date and priority - SUM of backlog days and COUNT
    my $SQL = q{
        SELECT
    ...
    while ( my @Row = $DBObject->FetchrowArray() ) {
        ...
        if ( $PriorityKey && exists $DayData{$Day} ) {
            $DayData{$Day}{$PriorityKey} = $BacklogDays;
            $GrandTotal{$PriorityKey} += $BacklogDays;
            $TicketCount{$PriorityKey} += $Count;
        }
    }
...
sub _GetWeeklyData {
    ...
    # Query ACTIVE tickets grouped by week and priority - SUM of backlog days and COUNT
    my $SQL = q{
        SELECT
    ...
    while ( my @Row = $DBObject->FetchrowArray() ) {
        ...
        if ( $PriorityKey ) {
            $WeekData{$Week}{$PriorityKey} = $BacklogDays;
            $GrandTotal{$PriorityKey} += $BacklogDays;
            $TicketCount{$PriorityKey} += $Count;
        }
    }
...
sub _GetMonthlyData {
    ...
    # Query ACTIVE tickets grouped by month and priority - SUM of backlog days and COUNT
    my $SQL = q{
        SELECT
    ...
    for my $month ($StartMonth..$EndMonth) {
        ...
        my $next_month_time = mktime(0, 0, 0, 1, $month, $Year - 1900);
        my $last_day_time = $next_month_time - 1;
        my @ld = localtime($last_day_time);
    ...
    while ( my @Row = $DBObject->FetchrowArray() ) {
        ...
        if ( $PriorityKey && exists $MonthData{$Month} ) {
            $MonthData{$Month}{$PriorityKey} = $BacklogDays;
            $GrandTotal{$PriorityKey} += $BacklogDays;
            $TicketCount{$PriorityKey} += $Count;
        }
    }
...
sub _GetQuarterlyData {
    ...
    # Query ACTIVE tickets grouped by quarter and priority - SUM of backlog days and COUNT
    my $SQL = q{
        SELECT
    ...
    for my $quarter (1..4) {
        ...
        use POSIX qw(mktime);
        my $next_month_time = mktime(0, 0, 0, 1, $end_month, $Year - 1900);
        my $last_day_time = $next_month_time - 1;
        my @ld = localtime($last_day_time);
        my $q_end = sprintf("%04d-%02d-%02d 23:59:59", $ld[5]+1900, $ld[4]+1, $ld[3]);
    ...
    while ( my @Row = $DBObject->FetchrowArray() ) {
        ...
        if ( $PriorityKey && exists $QuarterData{$Quarter} ) {
            $QuarterData{$Quarter}{$PriorityKey} = $BacklogDays;
            $GrandTotal{$PriorityKey} += $BacklogDays;
            $TicketCount{$PriorityKey} += $Count;
        }
    }
...
sub _GetYearlyData {
    ...
    # Query ACTIVE tickets grouped by year and priority - SUM of backlog days and COUNT
    my $SQL = q{
        SELECT
    ...
    while ( my @Row = $DBObject->FetchrowArray() ) {
        ...
        if ( $PriorityKey && exists $YearData{$Year} ) {
            $YearData{$Year}{$PriorityKey} = $BacklogDays;
            $GrandTotal{$PriorityKey} += $BacklogDays;
            $TicketCount{$PriorityKey} += $Count;
        }
    }
